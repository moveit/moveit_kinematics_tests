...
const double DEFAULT_SEARCH_DISCRETIZATION = 0.025f;
...
TEST(ArmIKPlugin, searchIK)
{
  ...
  for(unsigned int i=0; i < NUM_IK_TEST; ++i)
  {
    ...
    solution.clear();
    bool result = ikfast_test.kinematics_solver_->searchPositionIK(poses[0], seed, timeout, solution, error_code);
    ...
    {
      success++;
      result = ikfast_test.kinematics_solver_->getPositionIK(poses[0], solution, solution, error_code);
      EXPECT_TRUE(result);
    }
    else
    {
