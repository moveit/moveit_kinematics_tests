...
public:
  bool initialize() {
    ...
    kinematics_loader_.reset(
        new KinematicsLoader("moveit_core", "kinematics::KinematicsBase"));
    if (ph.getParam(PLUGIN_NAME_PARAM, plugin_name)) {
      try {
        ROS_INFO_STREAM("Loading " << plugin_name);
        kinematics_solver_.reset(
            kinematics_loader_->createUnmanagedInstance(plugin_name));
    ...
    // initializing plugin
    if (ph.getParam(GROUP_PARAM, group_name_) &&
        ph.getParam(TIP_LINK_PARAM, tip_link_) &&
        ph.getParam(ROOT_LINK_PARAM, root_link_) &&
        ph.getParam(JOINT_NAMES_PARAM, joints_)) {
      if (kinematics_solver_->initialize(ROBOT_DESCRIPTION_PARAM, group_name_,
                                         root_link_, tip_link_,
  ...
  void searchIKCallback(const geometry_msgs::Pose &ik_pose,
    ...
    solutions.resize(1);
    if (!kinematics_solver_->getPositionFK(link_names, joint_state,
                                           solutions)) {
...
TEST(IKFastPlugin, initialize) {
  ...
  EXPECT_TRUE(kinematics_test.root_link_ ==
              kinematics_test.kinematics_solver_->getBaseFrame());
  EXPECT_TRUE(kinematics_test.tip_link_ ==
              kinematics_test.kinematics_solver_->getTipFrame());
  std::vector<std::string> joint_names =
      kinematics_test.kinematics_solver_->getJointNames();
  EXPECT_EQ((int)joint_names.size(), kinematics_test.joints_.size());

  for (unsigned int i = 0; i < joint_names.size(); i++) {

    std::vector<std::string>::iterator pos =
        std::find(kinematics_test.joints_.begin(),
                  kinematics_test.joints_.end(), joint_names[i]);
    EXPECT_TRUE(pos != kinematics_test.joints_.end())
        << "Joint " << joint_names[i] << " not found in list";
  }
}
...
TEST(IKFastPlugin, getFK) {
  ...
  moveit_msgs::MoveItErrorCodes error_code;
  solution.resize(kinematics_test.kinematics_solver_->getJointNames().size(),
                  0.0);
  ...
  for (unsigned int i = 0; i < kinematics_test.num_fk_tests_; ++i) {
    ...
    poses.resize(1);
    succeeded = kinematics_test.kinematics_solver_->getPositionFK(
...
TEST(IKFastPlugin, searchIK) {
  ...
  for (unsigned int i = 0; i < kinematics_test.num_ik_tests_; ++i) {
    ...
    bool result_fk = kinematics_test.kinematics_solver_->getPositionFK(
        fk_names, fk_values, poses);
    ASSERT_TRUE(result_fk);
    solution.clear();
    kinematics_test.kinematics_solver_->searchPositionIK(
        poses[0], seed, timeout, solution, error_code);
    ...
    if (result) {
      EXPECT_TRUE(kinematics_test.kinematics_solver_->getPositionIK(
          poses[0], solution, solution, error_code));
    ...
    new_poses.resize(1);
    result_fk = kinematics_test.kinematics_solver_->getPositionFK(
        fk_names, solution, new_poses);
...
TEST(IKFastPlugin, searchIKWithCallback) {
  ...
  for (unsigned int i = 0; i < kinematics_test.num_ik_cb_tests_; ++i) {
    ...
    kinematics_test.kinematics_solver_->searchPositionIK(
        ...
        boost::bind(&KinematicsTest::searchIKCallback, &kinematics_test, _1, _2,
                    _3),
        error_code);
    bool result = error_code.val == error_code.SUCCESS;
...
TEST(IKFastPlugin, getIK) {
  ...
  for (unsigned int i = 0; i < kinematics_test.num_ik_tests_; ++i) {
    ...
    kinematics_test.kinematics_solver_->getPositionIK(poses[0], fk_values,
                                                      solution, error_code);
    ROS_DEBUG("Pose: %f %f %f", poses[0].position.x, poses[0].position.y,
              poses[0].position.z);
    ROS_DEBUG("Orient: %f %f %f %f", poses[0].orientation.x,
    ...
    if (error_code.val == error_code.SUCCESS) {
      ...
      ROS_ERROR_STREAM("getPositionIK failed on test "
                       << i + 1 << " for group "
                       << kinematics_test.kinematics_solver_->getGroupName());
      continue;
    }
...
TEST(IKFastPlugin, getIKMultipleSolutions) {
  ...
  for (unsigned int i = 0; i < kinematics_test.num_ik_multiple_tests_; ++i) {
    ...
    kinematics_test.kinematics_solver_->getPositionIK(
        poses, fk_values, solutions, result, options);
    ROS_DEBUG("Pose: %f %f %f", poses[0].position.x, poses[0].position.y,
              poses[0].position.z);
    ROS_DEBUG("Orient: %f %f %f %f", poses[0].orientation.x,
              poses[0].orientation.y, poses[0].orientation.z,
    ...
    if (result.kinematic_error == kinematics::KinematicErrors::OK) {
      EXPECT_GT(solutions.size(), 0) << "Found " << solutions.size()
                                     << " ik solutions.";
    ...
    for (unsigned int i = 0; i < solutions.size(); i++) {
      ...
      EXPECT_TRUE(kinematics_test.kinematics_solver_->getPositionFK(
          fk_names, solution, new_poses));
      EXPECT_NEAR(poses[0].position.x, new_poses[0].position.x, IK_NEAR);
      EXPECT_NEAR(poses[0].position.y, new_poses[0].position.y, IK_NEAR);
      EXPECT_NEAR(poses[0].position.z, new_poses[0].position.z, IK_NEAR);
